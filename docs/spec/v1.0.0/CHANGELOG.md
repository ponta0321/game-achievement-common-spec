...
**コア原則 6 → 5 → 4 への整理**
- 6 → 5: 「改竄耐性」を削除 (「sha256 で識別」は照合相手レジストリ未定義で
  実装不能、PNG/WebP エンコーダ差で sha256 はピクセル同一でも変動するため)
  - §7.3 を「同一性の判定について」(achievement_uid のみ) に書き換え
  - §10.4 を「取り込み時の運営審査」(運営ワークフロー責務) に書き換え
  - 機械的真正性が必要な場合は Trust spec で補完する設計
- 5 → 4: 「後方互換」をコア原則から削除 (公開直前修正)
  - 説明文「v1.0.0 で凍結、破壊的変更は v2.0.0 として並行発行」は
    バージョニング規約 (§11) であり、ユーザー価値を語るコア原則として
    不適切だった
  - コア原則 4 つ (ユーザー帰属 / アンチ独占 / 永続性 / 自己完結性) は
    すべてユーザー価値を語る統一感のある原則として整理
  - バージョニング規約は §11 で詳細記述済み、トップで重複させない
