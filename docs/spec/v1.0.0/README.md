...
| UserAchievement 件数 | ファイルサイズ目安 | 推奨経路 |
|---|---|---|
| ~1,000 件 | ~1-2 MB | Basic (単一ファイル) |
| ~10,000 件 | ~10-20 MB | Basic 可、Connected (pagination) も検討 |
| 10,000 件超 | 20 MB 超 | Connected / Verified の API 経路を強く推奨 |

ファイル経路で 20 MB を超える場合は **複数ファイルに分割** することを推奨:

- ファイル名規約: `export-part-001.json`, `export-part-002.json`, ...
- 各ファイルが独立した export.schema.json 準拠
- 分割の基準は実装サイト自由 (1 ファイル 5,000 件程度、または時系列で分割など)
- import 側は同じ操作で複数ファイルを受け入れる UI を提供
- `user_achievements` 配列の連結で全件復元可能 (重複は record_uid で検知)

大規模データの常用が想定されるサイトは Connected / Verified の API 経路 (transport-envelope の pagination) を実装することを推奨する。
