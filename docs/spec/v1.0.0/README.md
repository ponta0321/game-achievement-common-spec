...
UserAchievement は、他サイトに取り込まれたときに **元の Achievement レコードが取得できなくても表示可能** でなければならない。そのため、達成時点の表示情報スナップショットを埋め込む。
...
function export_for_user(user):
    ...
    for ua in db.user_achievements.where(owner = user, status = "approved"):
        ...
        records.append({
            ...
            achievement_snapshot: { title: ach.title, description: ach.description, thumbnail: ach.thumbnail },
            item_refs:            strip_internal_fields(ach.item_refs),  # local_id 等を除外
...
2. **画像 sha256 で照合** — `thumbnail.data_uri` のバイナリ sha256 を計算し、デフォルト実績ハッシュテーブルと突合
3. **整合性が取れない入力は pending で投入** — 即拒否ではなく、運営者が後で判断できる状態にする

詳細は内部実装仕様 `private/spec/import.md` を参照。
