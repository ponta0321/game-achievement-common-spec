...
バージョニング規約 (凍結・破壊的変更の方針) は §11 を参照。
...
├── export.schema.json                 ← ファイル交換用エンベロープ (Data spec、単一 JSON)
├── export-ndjson-meta.schema.json     ← NDJSON 形式のメタデータ行 (Data spec、大規模 export 向け)
...
└── examples/
    ...
    ├── export.example.json
    ├── export.example.ndjson           ← NDJSON 形式のサンプル
...
UserAchievement のデータ自体には **「誰のデータか」を示すフィールドが含まれない**。これは意図的なプライバシー保護設計であり、所有者の特定は **取り込みコンテキスト** (=「誰がこの取り込みを開始したか」) で行う。

**この章はファイル経路・API 経路の双方で実装者が踏みがちな落とし穴を明文化する。仕様だけ読んで実装するために必須の情報。**

#### 8.4.1 設計判断: なぜデータに owner を含めないか

| 案 | 採用? | 理由 |
|---|---|---|
| データに `owner_user_id` / `owner_handle` を含める | ❌ 不採用 | 第三者がデータを見るだけで「誰がどのサイトを使っているか」が漏れる。CC0 で配布される前提上、プライバシー漏洩リスクが大きい |
| 取り込みコンテキスト (OAuth 2 セッション or 手動 import 操作) で所有者を決定 | ✅ 採用 | データ自体は匿名のまま流通可能。各サイトが自前で所有者紐付け |

これは ActivityPub の `actor` フィールドや ATProto の DID とは異なる設計判断。本仕様は「**実績の事実そのもの**」を運ぶことに集中し、「誰が達成したか」は取り込み側の責務とする。

#### 8.4.2 ファイル経路 (Basic) でのユーザー紐付け

```
[サイト B] a さん@B が B 社にログイン中
[サイト B] a さん@B が import 画面でファイルをアップロード
[サイト B] アップロード操作のセッション = a さん@B
[サイト B] パース結果の UserAchievement[] を **a さん@B のアカウントに紐付け** て pending 投入
```

要件:
- 取り込み画面は **ログイン必須** とする (未ログインユーザーが他人の実績を取り込めないように)
- ファイルアップロードのセッションから所有者を決定する

#### 8.4.3 API 経路 (Connected / Verified) でのユーザー紐付け

OAuth 2 認可フローは **2 つのセッション** が並行して存在する:

| セッション | サイト | 表す主体 |
|---|---|---|
| (S1) | サイト B (取り込み先) | 「import を開始した B 社ユーザー」(= a さん@B) |
| (S2) | サイト A (取り込み元) | 「データ共有に同意した A 社ユーザー」(= a さん@A) |

取り込みコンテキストは **(S1) を所有者として扱う**。すなわち:

```
[サイト B] a さん@B がログイン → B 社セッション (S1) 発行
[サイト B] 「サイト A から取り込み」クリック → OAuth 2 authorize へリダイレクト
   ↓ ここで (S1) を維持する必要あり (後述 8.4.4)
[サイト A] a さん@A がログイン → A 社セッション (S2) 発行
[サイト A] 同意 → code 発行 → サイト B の callback にリダイレクト
[サイト B] code → access_token 交換 → user_achievements 取得
[サイト B] 取得したレコードを **(S1) の a さん@B のアカウントに紐付け** て投入
```

**重要**: A 社 access_token は「(S2) の a さん@A の実績にアクセスする権限」を表すだけで、「(S1) の B 社ユーザーが誰か」は教えてくれない。所有者は B 社が自前で (S1) から決定する。

#### 8.4.4 OAuth 2 callback でのセッション維持 (実装必須)

並行する 2 つのブラウザタブや、authorize 中のログアウト・ログイン切り替えで **(S1) が変わる/失われる** 可能性がある。対策:

1. **`state` パラメータに B 社内のセッション識別子を埋め込む** (定石)
   ```
   state = HMAC(server_secret, B 社セッション ID + nonce)
   ```
   callback で state を検証し、戻り先のセッションが authorize 開始時と同一であることを確認する

2. **callback 時にログインしていない / 別ユーザーになっていたら reject**
   - 「import を完了するには元のユーザー (a さん@B) で再ログインしてください」と促す

3. **取り込み開始時に元の B 社セッションをサーバー側ストレージに保存** (Redis 等)、callback で参照

これらは OAuth 2 標準 (RFC 6749) の範囲だが、本仕様の実装者が見落としがちなので明示する。

#### 8.4.5 profile endpoint との関係 (任意機能)

A 社が `/api/v1/profile` (Transport spec §5.4) を実装している場合、B 社は **(S2) のユーザー情報 (handle 等)** を取得できる。これを (S1) と一致確認に使う:

- **一致確認の用途**: 「a さん@A の handle = `alice@a-corp.com`」と「a さん@B の handle = `alice@b-corp.com`」を表示し、ユーザーに「同一人物として紐付けてよいか?」最終確認を求める UI を出せる
- **機械的判定には使わない**: handle が違っても同一人物の可能性はある (匿名アカウント、別 namespace 等)。最終決定は (S1) のユーザーに委ねる

profile が無いサイトの場合は **(S1) に盲目的に紐付ける**。これでも仕様準拠として valid。

#### 8.4.6 取り込み後の訂正 (実装推奨)

a さんが「これは私のデータじゃない」と気付いた場合の訂正フロー (本仕様では規定しないが推奨実装):

- 取り込んだ UserAchievement の所有者付け替え (B 社内で別ユーザーに移行)
- 取り込みごとキャンセル (該当 record_uid をまとめて削除)
- 取り込み履歴の閲覧 (どの A 社セッションから何件 import したか)

これらは README §13 の `imported_at` / `imported_from` 等の内部 DB フィールドを使って実装する。

#### 8.4.7 アカウント削除後シナリオ

a さんが A 社のアカウントを削除した後、別人が同じ handle を取得 → B 社に取り込みを試みるリスク:

- **a さんが過去に行った取り込みは B 社 DB に残る** (record_uid + imported_at で履歴保持)
- 新たな取り込み要求は **新 OAuth 2 セッションを別人として扱う** ことになる
- B 社は「同一 A 社 handle だが過去取り込みと別人 (異なる timestamp 等)」を検出可能だが、**機械的に同一人物保証はしない** (運営審査依存)

これはなりすまし対策の限界であり、本仕様は OAuth 2 標準を超えた保証はしない。Verified サイトは「**OAuth 2 同意 + 運営審査**」の二段構えで対処すること。
...
### 9.1 ファイル経路のサイズ目安と形式選択

1 UserAchievement のサイズは平均 0.8-1.5 KB (thumbnail data_uri 込み)。件数とファイルサイズの目安:

| UserAchievement 件数 | ファイルサイズ目安 | 推奨形式 |
|---|---|---|
| ~1,000 件 | ~1-2 MB | JSON (単一ファイル) |
| ~10,000 件 | ~10-20 MB | JSON または NDJSON |
| 10,000 件超 | 20 MB 超 | **NDJSON 必須** または Connected/Verified の API 経路 |

### 9.2 NDJSON 形式 (大規模 export 向け)

ヘビーユーザー (Steam で数万件等) のデータを単一 JSON で運ぶと、JSON.parse でメモリが数倍に膨らみ、ブラウザがクラッシュする。本仕様では **NDJSON (Newline Delimited JSON)** を正式形式として提供する。

- **拡張子**: `.ndjson` (推奨) または `.jsonl`
- **構造**: 1 行 = 1 JSON オブジェクト、行間を `\n` (LF) で区切る
- **1 行目**: メタデータ行 (`export-ndjson-meta.schema.json` 準拠)
- **2 行目以降**: UserAchievement[] (`user_achievement.schema.json` 準拠、1 行 1 件)

メタデータ行のスキーマ: [export-ndjson-meta.schema.json](export-ndjson-meta.schema.json)。サンプル: [examples/export.example.ndjson](examples/export.example.ndjson)。

#### サンプル

```
{"_meta":true,"schema_version":"1.0.0","source_site":"https://example.com/","exported_at":"2026-06-25T11:48:26+09:00","format":"ndjson","expected_count":2}
{"record_uid":"example.com:uach:88012","achievement_uid":"example.com:ach:1234", ...}
{"record_uid":"example.com:uach:88013","achievement_uid":"example.com:ach:1235", ...}
```

#### メリット

- **ストリーミング処理**: 行単位で読み書きでき、巨大ファイルでもメモリに収まる
- **1 行の reject が容易**: バッチ全体を破棄せず、不正な行だけ skip + warning
- **既存ツールで処理可能**: `jq` / `awk` / `grep` / pandas / Spark 等
- **HTTP ストリーミングと相性が良い**: chunked transfer encoding でストリーミング送信可能

#### 実装の最小コード

```python
# Export
def write_ndjson(file, user, records):
...
def read_ndjson(file):
    ...
    for line in file:
        ...
        try:
            ...
            log_warning(f"skipped invalid line: {line[:80]}")
```

#### NDJSON と単一 JSON の使い分け

| 状況 | 推奨形式 |
|---|---|
| 通常ユーザー (~1,000 件) | 単一 JSON (export.schema.json) — 人が読みやすい |
| ヘビーユーザー (10,000 件超) | NDJSON — ストリーミング処理可能 |
| API 経路 (Connected / Verified) | transport-envelope + pagination — 形式問わず |

実装サイトは **両形式を export ボタンの選択肢として提供することを推奨** する。import 側は **両形式を受け入れる** ことを推奨 (拡張子または先頭行の `_meta` で判別)。

### 9.3 ファイル分割 (任意)

NDJSON 形式でも 1 ファイルが大きすぎる場合 (例: 500 MB 超)、ファイル分割を推奨:

- ファイル名規約: `export-part-001.ndjson`, `export-part-002.ndjson`, ...
- 各ファイルが独立した NDJSON 準拠 (メタデータ行 + UserAchievement 行)
- 各ファイルのメタデータ行に `expected_count` を入れることで import 側が進捗表示可能
- import 側は同じ操作で複数ファイルを受け入れる UI を提供
- 全ファイル連結で全件復元可能 (重複は record_uid で検知)
...
`current_user` は **取り込みを開始した自サイトユーザー** (= ファイル経路ならアップロード操作のセッションのユーザー、API 経路なら OAuth 2 callback 戻り時のセッションのユーザー)。データ自体には所有者識別子が含まれないため、コンテキストで決定する (§8.4 参照)。

```pseudo
function import_export(file, current_user):
    # 前提: current_user はログイン必須、API 経路では state 検証通過後に確定
