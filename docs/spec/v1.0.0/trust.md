...
- 90 日経過後、引退鍵は `jwks-archive.json` (§6.1) に移動。`jwks.json` からは削除

#### 3.3.2 鍵漏洩時の緊急ローテーション

- 漏洩した kid を **即時 `jwks.json` から削除** (90 日重複公開ルールの例外)
- **`jwks-archive.json` にも移動しない** (検証通過させない)
- 全 access_token revocation
- セキュリティアドバイザリを発出
- 受信側 (B 社等) は警告を受けて、漏洩 kid で署名された **過去の JWS も全て無効** として扱う:
  - 該当 record を pending に戻す (運営審査経由で再 approve)
  - または該当 record を一旦削除し、ユーザーに再取り込みを促す
- 漏洩公表前に B 社が信頼して approved 投入した record は **B 社の運営判断** で事後処理 (取り消し / 維持 / 個別審査)
...
[サイト B] サイト A からの応答受信
       ...
       4. kid に一致する公開鍵を選択
          見つからない場合 → 4'. へフォールバック
       ↓
       4'. (fallback) jwks_archive_uri が well-known で公開されていれば
          jwks-archive.json を取得し、kid に一致する公開鍵を再検索
          archive にも無ければ → reject (key_not_found)
       ↓
