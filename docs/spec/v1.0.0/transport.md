...
uid はコロン `:` を含むため URL エンコードする必要がある (例: `example.com:ach:default:cleared:17602` → `example.com%3Aach%3Adefault%3Acleared%3A17602`)。

レスポンス: achievement.schema.json 準拠 JSON。

### 5.4 `GET /api/v1/profile`

`profile:read` scope で利用可能。トークンを発行したユーザーの基本プロファイルを返す。取り込み先サイトでユーザー紐付けを補強するために使う。
...
```json
{
  ...
  "handle_lang": "en",
  "display_name": "Alice",
  "display_name_i18n": { "ja": "アリス" },
  "site_user_id": "alice@a-corp.com",
  "primary_lang": "en"
}
```
...
| フィールド | 必須 | 意味 |
|---|---|---|
| `handle` | ✅ | サイト内のユーザーハンドル (英数字推奨) |
| `handle_lang` | — | handle の言語ヒント (BCP 47) |
| `display_name` | — | 表示名 |
| `display_name_i18n` | — | 表示名の翻訳マップ (Data spec の i18n マップと同形式) |
| `site_user_id` | — | サイト内一意のユーザー ID (内部 ID をエクスポートする場合) |
| `primary_lang` | — | ユーザーの主言語 (UI フォールバック用) |

このエンドポイントは Connected 適合の **任意実装** だが、ユーザー紐付け精度を上げるため実装を強く推奨する。
受信側はこの情報を「現在ログイン中ユーザーと一致するか」の参考にしてよいが、**機械的な同一性判定には使わない** (なりすまし対策は OAuth 2 同意フローと運営審査で行う)。
...
```json
{
  ...
  "endpoints": {
    ...
    "profile": "https://example.com/api/v1/profile"
  },
  "supported_scopes": [
    "achievements:read",
    "achievements:write",
...
### 10.3 OAuth 2 client 登録

#### 10.3.1 v1.0 で推奨される方式: 手動事前登録 (オフバンド)

サイト A の OAuth 2 サーバーには、サイト B を **`client_id` / `client_secret` (Confidential Client) または `client_id` のみ (Public Client + PKCE)** として事前登録する必要がある。

v1.0 ではこの登録手順を仕様化しない。実装サイトは以下のいずれかを採用する:

1. **管理画面での手動登録** — 運営者が GitHub Issues / Discussions / メール等で連絡を取り、互いに `client_id` を登録
2. **動的登録 (RFC 7591)** — 任意採用。well-known に `registration_endpoint` を追加して機械的登録を許可

各サイト運営者向けに「他サイトを連携先として登録」「自サイトに登録された他サイト一覧」「ユーザーの同意履歴」管理画面が必要。

#### 10.3.2 将来 v1.1 以降での検討

- **RFC 7591 動的登録の標準化** (現在は任意)
- **コミュニティ運営の連携先レジストリ** (well-known list)

これらは v1.0 では規定しない。手動運用が現実解。
